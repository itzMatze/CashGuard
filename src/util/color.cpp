...
uint32_t Color::get_hex_color() const
{
	...
	uint32_t hex_color = std::clamp(static_cast<int64_t>(a * 255.999f), int64_t(0), int64_t(255));
	hex_color <<= 8;
	...
	hex_color <<= 8;
	hex_color += std::clamp(static_cast<int64_t>(r * 255.999f), int64_t(0), int64_t(255));
	return hex_color;
}
...
std::string Color::to_string() const
{
	...
	uint8_t b = (hex_color >> 16) & 0xff;
	uint8_t g = (hex_color >> 8) & 0xff;
	uint8_t r = hex_color & 0xff;
